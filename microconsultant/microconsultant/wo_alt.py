...
def update(doc_name):
    ...
    for d in self.get('required_items'):
        if d.alternate == 0:
            if d.available_qty_at_source_warehouse < d.required_qty:
                ...
                # Sorted stock in ascending order
                sorted_stock = {k: v for k, v in sorted(stock_dict.items(), key=lambda v: v[1])}
                ...
                for x, y in sorted_stock.items():
                    if x in altic:
                        if rq <= 0:
                            ...
                            rqp = rq - y
                            if rqp <= 0:
                                new_qty = d.required_qty - rq
                                # Direct DB update for required_qty
                                frappe.db.sql("""UPDATE `tabWork Order Item` SET required_qty=%s WHERE name=%s""", (new_qty, d.name))
                                new_item = frappe.get_doc({
                                    ...
                                    'available_qty_at_source_warehouse': frappe.db.get_value("Bin", {"warehouse": d.source_warehouse, "item_code": x}, "actual_qty"),
                                    'alternate': 1,
                                    'alternate_of': d.item_code,
                                    'idx': d.idx
                                })
                                new_item.insert(ignore_permissions=True)
                                stock_dict[x] = y - rq
                            elif y > 0:
                                new_qty = d.required_qty - y
                                # Direct DB update for required_qty
                                frappe.db.sql("""UPDATE `tabWork Order Item` SET required_qty=%s WHERE name=%s""", (new_qty, d.name))
                                new_item = frappe.get_doc({
                                    'doctype': 'Work Order Item',
    ...
    # Commit the changes directly to the database
    frappe.db.commit()
