...
def add_items(self, method):
	...
	for d in items[:]:
		...
		stock = 0.0
		alt_stock = 0.0
		stocks = frappe.db.sql_list("""SELECT actual_qty FROM `tabBin` WHERE item_code=%s and warehouse !=%s  and projected_qty ='0'""",(d.item_code,d.warehouse))
		for k in stocks:
			stock = stock +k
		...
		for a in altic:
			alt_stocks = frappe.db.sql_list("SELECT actual_qty FROM `tabBin` WHERE item_code=%s  and projected_qty ='0'",a)
			for o in alt_stocks:
				...
				for d in it[:]:
					for z in itm:
						if d.item_code == z:
							...
							qty_or=0.0
							alt_stock=0.0
							alt_stocks = frappe.db.sql_list("""SELECT actual_qty FROM `tabBin` WHERE item_code=%s  and projected_qty ='0'""",p)
							for k in alt_stocks:
